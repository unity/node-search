...
exports.NodeSearch = function() {
	return {
		...
		rawVectorKeywordIndex:{},
		rawVectorKeywordIndexLength:0,
		index: function(key,doc,callback){
			var self = this;
			var addWords = [];
			var rawAddWords = [];
			var uniquesList = findUniques(doc);

			uniquesList.tokenized.forEach(function(word){
			...
			uniquesList.raw.forEach(function(word){
				if( !self.rawVectorKeywordIndex.hasOwnProperty(word) ){
					self.rawVectorKeywordIndex[word] = self.rawVectorKeywordIndexLength;
					self.rawVectorKeywordIndexLength++;
					rawAddWords.push(word);
				}
			});
			...
			if( rawAddWords.length != 0 ) {
				Object.keys(self.rawDocs).forEach(function(column){
					self.rawDocs[column].forEach(function(item){
						rawAddWords.forEach(function(word){
							item.data.push(0);
						});
					});
				});
			...
			if( typeof(doc) == "object" ) {
				...
				for( var field in doc ){
					...
					var rawVector = makeVector(key, doc[field],self.fieldWeights[field]||1,self.vectorKeywordIndex,self.vectorKeywordIndexLength,true);
					if( this.docs.hasOwnProperty(field) ){ 
						this.docs[field].push(vector);
						this.rawDocs[field].push(rawVector);
					}else{
						this.docs[field] = [vector];
						this.rawDocs[field] = [rawVector];
					}
				}
				this.docs.all.push( makeVector(key,fieldsData,1,self.vectorKeywordIndex,self.vectorKeywordIndexLength));
				this.rawDocs.all.push( makeVector(key,fieldsData,1,self.rawVectorKeywordIndex,self.rawVectorKeywordIndexLength, true));
			}else{
				this.docs.all.push( makeVector(key,doc,1,self.vectorKeywordIndex,self.vectorKeywordIndexLength));	
				this.rawDocs.all.push( makeVector(key,doc,1,self.rawVectorKeywordIndex,self.rawVectorKeywordIndexLength, true));
			}
		},
		...
		query: function(string,fields,callback){
			...
			var docs, vector;
			var words = accentsTidy(string).split(" ");
			var count = 0;
			var total = 0;
			var completed = 0;
			var falseMatches = [];
			...
			rawVector = makeVector("",words.join(" "),1,this.rawVectorKeywordIndex,this.rawVectorKeywordIndexLength, true);
			...
			if( fields != null){
				...
				if( !this.docs.hasOwnProperty(fieldsName) ) {
					...
					rawDocs = this.rawDocs[fieldsName];
				}
			}else{
				docs = this.docs.all;
				rawDocs = this.rawDocs.all;
			}
          ...
          var rawResult = cosine(rawVector,rawDocs[i]);

          if( rawResult != 0 ){ // filter out items that dont match at all
           rawResults.push({key:doc.key, rank:rawResult});
          }

				},
				function(){ // TODO: what if there are millions of results, this sort will be slow.
					callback({
					  phonetic: results.sort(function (a, b) { return ((b.rank - a.rank)) }),
					  raw: rawResults.sort(function(a,b){ return((b.rank - a.rank)) })
				  });
				}
			);
		},
...
var stopWords = exports.stopWords = ["alors","au","aucuns","aussi","autre","avant","avec","avoir","bon","car","ce","cela","ces","ceux","chaque","ci","comme","comment","dans","des","du","dedans","dehors","depuis","deux","devrait","doit","donc","dos","droite","début","elle","elles","en","encore","essai","est","et","eu","fait","faites","fois","font","force","haut","hors","ici","il","ils","je	juste","la","le","les","leur","là","ma","maintenant","mais","mes","mine","moins","mon","mot","même","ni","nommés","notre","nous","nouveaux","ou","où","par","parce","parole","pas","personnes","peut","peu","pièce","plupart","pour","pourquoi","quand","que","quel","quelle","quelles","quels","qui","sa","sans","ses","seulement","si","sien","son","sont","sous","soyez	sujet","sur","ta","tandis","tellement","tels","tes","ton","tous","tout","trop","très","tu","valeur","voie","voient","vont","votre","vous","vu","ça","étaient","état","étions","été","être","un","une","de", "le","la","les","n", "ne","se","cet","l","s","va","d","qu","hui"];
...
function findUniques(doc){
	...
	//Remove common words which have no search value
	var cleanString = removeStopWords(vocabularyString);
	var vocabularyList = tokenizer.process(cleanString);

	return {tokenized: removeDuplicates(vocabularyList), raw:removeDuplicates(cleanString)};
}

// Make a vector that has values for each instance of the words contained in each document
function makeVector(key,string,fieldWeight,vectorKeywordIndex,vectorKeywordIndexLength,raw){
	fieldWeight = fieldWeight||0;
	var vector = Vector(vectorKeywordIndexLength);
	if(raw){
	  var wordList = removeStopWords(string);
	} else {
...
function clean(string){
	var st = string+""
	return accentsTidy(st).replace(/[\)\(\:\'\,\.\-_&]/g, " ").replace(/\s\s+/g, " ").replace('\s[A-Z,a-z]\s').toLowerCase();
}
